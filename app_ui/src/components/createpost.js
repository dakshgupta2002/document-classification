...
export default function CreatePost({ createP, setCreateP, userId }) {
    ...
    const handlePost = () => {
        ...
        const imageRef = ref(storage, `${userId}/${docId}.${document.name.substring(document.name.lastIndexOf('.') + 1)}`)
    ...
    const uploadToFirestore = () => {
        getDownloadURL(ref(storage, `${userId}/${docId}.${document.name.substring(document.name.lastIndexOf('.') + 1)}`))
            .then(async (url) => {
                // console.log(url);
                await firebase
                    .firestore()
